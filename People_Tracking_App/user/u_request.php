...
if(isset($_REQUEST['submit'])){ // PHP Signup Validation Starts
    ...
    if(strlen($reason)<=0){
     ...
     if(count($errorArray)<=0 || $valid == true){
        ...
        $insertRequest = "INSERT INTO request(reason,description,status,from_date,to_date,rf_key,requested_on,rfid,user_id,from_location,to_location,head_count)
        VALUES('$reason','$desc','PENDING','$from','$to','$fetched_value','$today','$rfid','$userid','$fromLoc','$toLoc','$headCount')";
        // echo $reason,$desc,"PENDING",$from,$to,$fetched_value,$today,$rfid,$userid,$fromLoc,$toLoc,$headCount;
